...
function validate(values: FormValues): FieldErrors {
  ...
  else if (!EMAIL_RE.test(values.email)) errors.email = 'Diese E-Mail-Adresse scheint nicht zu stimmen.'
  if (!values.message.trim()) errors.message = 'Bitte schreiben Sie eine kurze Nachricht.'
...
export type ContactFormLabelInput = Partial<Record<keyof ContactFormLabels, string | null>>
...
export function ContactForm({ labels }: { labels?: ContactFormLabelInput | null }) {
  ...
  if (status === 'success') {
    return (
      ...
      >
        <CheckCircle size={44} weight="duotone" className="text-brand-700 dark:text-brand-300" aria-hidden />
        <h3 className="mt-4 text-xl font-bold tracking-tight text-zinc-900 dark:text-white">
          {t.successTitle}
        </h3>
        <p className="mt-2 max-w-sm leading-relaxed text-zinc-500 dark:text-zinc-400">{t.successText}</p>
      </motion.div>
    )
  }
    ...
    <form onSubmit={handleSubmit} noValidate>
      <div className="grid gap-5 sm:grid-cols-2">
        <div className="flex flex-col gap-2">
          <label htmlFor="cf-name" className="text-sm font-medium text-zinc-800 dark:text-zinc-200">
            {t.nameLabel}
          </label>
          <input
            id="cf-name"
          ...
          />
          {errors.name && (
            <p id="cf-name-error" className="text-sm text-red-600 dark:text-red-400">
              {errors.name}
            </p>
          )}
        </div>
        ...
        <div className="flex flex-col gap-2">
          <label htmlFor="cf-email" className="text-sm font-medium text-zinc-800 dark:text-zinc-200">
            {t.emailLabel}
          </label>
          <input
            id="cf-email"
          ...
          />
          {errors.email && (
            <p id="cf-email-error" className="text-sm text-red-600 dark:text-red-400">
              {errors.email}
            </p>
          )}
        </div>
        ...
        <div className="flex flex-col gap-2">
          <label htmlFor="cf-phone" className="text-sm font-medium text-zinc-800 dark:text-zinc-200">
            {t.phoneLabel}
          </label>
          <input
            id="cf-phone"
        ...
        <div className="flex flex-col gap-2">
          <label htmlFor="cf-service" className="text-sm font-medium text-zinc-800 dark:text-zinc-200">
            {t.topicLabel}
          </label>
          <select
            id="cf-service"
      ...
      <div className="mt-5 flex flex-col gap-2">
        <label htmlFor="cf-message" className="text-sm font-medium text-zinc-800 dark:text-zinc-200">
          {t.messageLabel}
        </label>
        <textarea
          id="cf-message"
        ...
        />
        {errors.message && (
          <p id="cf-message-error" className="text-sm text-red-600 dark:text-red-400">
            {errors.message}
          </p>
        )}
      </div>
      ...
      <div className="mt-5 flex flex-col gap-2">
        ...
        {errors.consent && <p className="text-sm text-red-600 dark:text-red-400">{errors.consent}</p>}
      </div>
      ...
      <AnimatePresence>
        {status === 'error' && (
          ...
          >
            <WarningCircle size={20} weight="bold" className="mt-px shrink-0" aria-hidden />
